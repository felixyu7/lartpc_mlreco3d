...
__device__ T bilinear_interpolate(const T* bottom_data,
  ...
  T v5 = bottom_data[z_low * width + x_low];
  T v6 = bottom_data[z_low * width + x_high];
  T v7 = bottom_data[z_high * width + x_low];
  T v8 = bottom_data[z_high * width + x_high];
    
  T w1 = hy * hx, w2 = hy * lx, w3 = ly * hx, w4 = ly * lx, w5 = hz * hx, w6 = hz * lx, w7 = lz * hx, w8 = lz * lx;
...
__global__ void RoIAlignForward(const int nthreads, const T* bottom_data,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    T roi_start_d = offset_bottom_rois[3] * spatial_scale;
    T roi_end_w = offset_bottom_rois[4] * spatial_scale;
    T roi_end_h = offset_bottom_rois[5] * spatial_scale;
    T roi_end_d = offset_bottom_rois[6] * spatial_scale;
    ...
    for (int iy = 0; iy < roi_bin_grid_h; iy ++) // e.g., iy = 0, 1
    {
      ...
      for (int ix = 0; ix < roi_bin_grid_w; ix ++)
      {
        ...
        {
            const T z = roi_start_d + pd * bin_size_d + static_cast<T>(iz + .5f) * bin_size_d / static_cast<T>(roi_bin_grid_d);
