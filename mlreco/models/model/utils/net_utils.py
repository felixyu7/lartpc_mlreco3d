...
def compare_grid_sample():
    ...
    input = Variable(torch.randn(N, C, H, W).to(torch.device(cfg.MODEL.DEVICE)), requires_grad=True)
    input_p = input.clone().data.contiguous()

    grid = Variable(torch.randn(N, H, W, 2).to(torch.device(cfg.MODEL.DEVICE)), requires_grad=True)
    grid_clone = grid.clone().contiguous()

    out_offcial = F.grid_sample(input, grid)
    grad_outputs = Variable(torch.rand(out_offcial.size()).to(torch.device(cfg.MODEL.DEVICE)))
    ...
    crf = RoICropFunction()
    grid_yx = torch.stack([grid_clone.data[:,:,:,1], grid_clone.data[:,:,:,0]], 3).contiguous().to(torch.device(cfg.MODEL.DEVICE))
